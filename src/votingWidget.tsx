...
import { summaryOfVote, summaryOfVotingOptions, UserVote, VotingOptions, WidgetUser } from "./voting"

declare global {
    interface Array<T> {
        groupBy<K>(fn: (v: T) => K): Map<K, T[]>
        random(): T
    }
}
...
function VotingInfo({
    votingOptions,
    votings
}: { votingOptions: VotingOptions, votings: UserVote[] }) {
    const totalVotes = votings.length
    const avgVotes = (() => {
        switch (votingOptions.kind) {
            case 'numeric': return "ø " + (
            ...
            case 'categorical': return undefined
        }
    })()
    ...
    return (
        ...
        >
            ...
            >
                <Text fontSize={12} fontWeight={400} fill='#054F31'>{`${totalVotes} ${totalVotes == 1 ? 'Vote' : 'Votes'}`}</Text>
                {avgVotes ? ([
                    <Spacer size={10} />,
                    <Frame height={18} width={1} fill='#D1FADF' />,
                    <Spacer size={10} />,
                    <Text fontSize={12} fontWeight={400} fill='#054F31'>{avgVotes}</Text>
                ]) : undefined}
            </AutoLayout>
        </AutoLayout>
    )
...
function VotingResult({ votings }: { votings: UserVote[] }) {
    const totalVotes = votings.length
        ...
        return (
            ...
            >
                <AutoLayout
                    ...
                    width={'fill-parent'}
                >
                    <Text fontSize={14} fontWeight={600} fill='#101828'>{summaryOfVote(option)}</Text>
                ...
                <Text fontSize={12} fontWeight={400} fill='#101828' width='fill-parent' horizontalAlignText='right'>{userVotes.length} {userVotes.length == 1 ? 'Vote' : 'Votes'}</Text>
            </AutoLayout>
        )
    })
}
...
function AvatarSection(
    ...
    const rows: WidgetUser[][] = new Array()
        ...
        >
            {
                rows.map(r => (
                    ...
                    >
                        {
                            r.map(u => <Avatar photoUrl={u.userPhotoUrl} opacity={1.0} />)
                        }
                    </AutoLayout>
                ))
            }
...
export function Avatar({
    ...
    return (
        <Image width={24} height={24} src={photoUrl} cornerRadius={24} opacity={opacity} />
...
export function Button({
    ...
    return (
        <AutoLayout
            ...
            cornerRadius={8}
            fill='#C8CCE5'
            onClick={() => { return new Promise<void>(onClick) }}
        >
            <Text
